...
@test "identical chezmoi applies rerun mise, Yeet, then Moka adapter sync" {
  ...
  export CODEX_HOME="$destination_dir/.codex"
  export OPENCODE_CONFIG_DIR="$destination_dir/.config/opencode"
...
@test "Moka adapter sync force-migrates legacy package-owned adapters" {
  legacy_root="$BATS_TEST_TMPDIR/opencode"
  mkdir -p "$legacy_root/commands"
  cat >"$legacy_root/commands/moka-execute.md" <<'LEGACY'
<!-- Generated by @oisin-ee/moka. -->
<!-- @oisin-ee/moka:host=opencode -->
LEGACY
...
STUB
  ...
  run env OPENCODE_CONFIG_DIR="$legacy_root" CLAUDE_CONFIG_DIR="$BATS_TEST_TMPDIR/claude" bash "$rendered"

  [ "$status" -eq 0 ]
  [[ "$output" == *"Migrating legacy Moka adapter ownership"* ]]
